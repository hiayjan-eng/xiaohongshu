...
const previewDir = resolve(repoRoot, "release-artifacts", "extension-beta-preview-v0.2.5");
const previewZip = resolve(repoRoot, "release-artifacts", "collection-revival-extension-beta-preview-v0.2.5.zip");
...
if (!existsSync(previewZip)) throw new Error("Preview 0.2.5 zip is missing.");
if (manifest.version !== "0.2.5") throw new Error("Preview manifest must be 0.2.5.");
...
if (!profile.includes("Preview 测试包 0.2.5-preview") || !profile.includes(`${previewOrigin}/old-import`)) throw new Error("Preview build profile must identify the 0.2.5-preview test package and exact import target.");
