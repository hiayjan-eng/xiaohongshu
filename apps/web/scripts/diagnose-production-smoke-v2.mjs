...
const keys = ["collection-revival-storage-bootstrap:v1", "collection-revival-system:v1", "collection-revival-theme", "collection-revival-achievements"];
...
async function snapshot(page) {
  return page.evaluate(async (storageKeys) => ({
    ...
    main: Boolean(document.querySelector("main")), nav: Boolean(document.querySelector("nav")),
...
try {
  ...
  page.on("response", (response) => { if (["document", "script", "stylesheet"].includes(response.request().resourceType())) report.responses.push({ type: response.request().resourceType(), url: response.url(), status: response.status() }); });
  ...
  await page.addInitScript(() => window.addEventListener("unhandledrejection", (event) => { (window.__phase2UnhandledRejections ??= []).push(String(event.reason)); }));
...
finally { await browser.close(); report.finishedAt = new Date().toISOString(); await writeFile(resolve(outputDirectory, "diagnostic.json"), JSON.stringify(report, null, 2)); }
