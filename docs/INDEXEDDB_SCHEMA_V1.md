...
## Task 8 补充：Runtime 元数据与 Activation Journal

Task 8 保持数据库 schemaVersion 1，不新增 Store。为完整恢复 AppState，`settings` 需要保存 `app.user`、`app.schemaVersion` 和各实体集合的 order manifest；现有 legacy normalized Snapshot 尚未生成这些记录，因此在 Task 8B 补齐前不能正式激活。

Activation Journal 复用 `migrationMetadata` Store，以 `recordType: activation` 与迁移执行记录区分。启动选择仍使用独立 Bootstrap Marker；settings 中的镜像不能单独决定 active backend。当前 `activeStorageSwitched` 只在 IndexedDB boot、hydrate 和 integrity 验证成功后的 commit transaction 中写 true。

本决定避免在首次 activation 临界路径中叠加 v1 到 v2 的 versionchange 风险。未来确需新增 Store 或 index 时，再独立设计 v2。
- 文本迁移版本
- 数据迁移偏好
- 是否显示完成动效等长期用户设置
