...
## Task 8C Runtime 写入边界

Web 默认 authority 仍是 LocalStorageRuntime。RuntimePersistCoordinator 接入 StorageWriteGate：预检通知先阻止新写入，再等待已经接受的写入完成；prepared 状态继续冻结。普通启动只读 Bootstrap Marker，缺失或 legacy_active 继续 LocalStorageRuntime，activation_prepared/recovery_required 则在启动层阻断普通 UI。Task 8C 不把 IndexedDbRuntime 作为 App authority，也不做 fallback、双写或 reload。
## Task 8D 正式 Runtime 选择

`AppBootstrap` 现按 Marker 严格选择后端：缺失/legacy 使用 `LocalStorageRuntime`；prepared 阻断普通写入；activating/active 使用 `IndexedDbRuntime`；损坏或冲突进入启动级 Recovery。正式提交后 AppState、theme 与 achievements 只写 IndexedDB，三个 legacy 产品 key 保持字节不变。旧标签页收到 `collection-revival-storage-runtime:v1` 通知后关闭写 gate，刷新后重新 hydrate；没有双写或失败 fallback。
