...
generation guard 会忽略过期异步结果；StrictMode 的清理通过微任务和代次检查避免第一次探测挂载误关掉仍在使用的 Runtime。直接访问 `/settings/data-migration` 会绕过普通业务启动门，继续使用 Task 7 的只读迁移页面，因此损坏主 AppState 时仍可检查和下载原始备份。该路由使用不依赖 AppState 的轻量导航壳，保留 Task 7B 执行中离开确认，不会为了恢复侧栏而提前 hydrate 产品数据。
...
最终验证结果：storage-runtime `22 tests / 73 assertions`，storage-service `165 tests / 769 assertions`，全仓 typecheck 通过，生产 build 通过，Web E2E `122 passed`。完整回归曾出现一次既有移动端持久化观察窗口超时，单独重跑通过，最终完整套件全绿，记录为非阻塞 flaky。
