...
const categoryRules: CategoryRule[] = [
  ...
  { category: "AI 与效率", subCategory: "AI 工具", entityType: "tool", terms: ["AI工具", "AI 工具", "ChatGPT", "Claude", "Gemini", "Midjourney", "提示词", "prompt", "工作流", "自动化", "智能体", "插件", "工具清单", "SOP", "效率", "Notion", "Excel", "飞书", "办公"], intent: "用户可能想复现一个工具用法、提示词或工作流，并把它用进日常工作", whyThisCategory: "命中了 AI 工具、效率流程、自动化或 SOP 相关线索" },
...
function generateActionCard(category: Category, subCategory: string, input: ShareInput, keywords: string[], entities: EntityTag[], confidence: ClassificationConfidence, intent: string): ActionCardDraft {
  ...
  if (category === "AI 与效率" || category === "技能学习") {
    ...
    return card(common, isAi ? "复现卡" : "练习卡", isAi ? `把“${topic}”里的一个工具步骤复现出来。` : `用“${topic}”完成一次最小练习。`, isAi ? "你可能想把这个工具或工作流真正用到自己的任务里。" : "你可能想学习这个技能，但需要先做一次小练习确认入口。", isAi ? `打开原帖，找到第一个工具名和第一个操作步骤；今天只复现这一步，留下 1 张截图或 1 段可复用 prompt。` : `打开原帖，找到作者给的第一个练习动作；今天只做这一小步，并留下 1 个截图、片段或练习记录。`, isAi ? ["工具名", "第一个操作步骤", "示例输入", "示例产出"] : ["练习步骤", "示例作品", "注意事项", "完成标准"], isAi ? "1 张截图、1 段 prompt 或 1 个小自动化步骤" : "1 个小作品、练习截图或 3 行练习记录", "20分钟", "中", isAi ? "复现出原帖中的第一个可见结果，并保存截图或 prompt。" : "完成第一个练习动作，并记录哪里卡住。", "不要一口气学完整教程，也不要先收藏更多同类内容。", "如果不知道工具名或练习步骤，先打开原帖补齐这两个字段再重新生成。", "如果第一个步骤可用，再把它整理成自己的 SOP 或 3 天练习。", [task("定位第一步", isAi ? "找出工具名、入口和第一个操作步骤。" : "找出作者建议的第一个练习动作。", "6分钟"), task("只复现一次", isAi ? "按原帖做出第一个示例结果。" : "完成一个最小练习，不追求完美。", "10分钟"), task("保存证据", isAi ? "保存截图或 prompt，方便下次继续。" : "写下 1 句卡点和 1 句下次要练什么。", "4分钟")], { 学习目标: isAi ? "复现一个可用工具步骤" : "完成一次最小技能练习" });
  }
